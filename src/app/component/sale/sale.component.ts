...
export class SaleComponent implements OnInit {
  ...
  saleData: any;
  saleAmount: any;
  constructor(private fb: FormBuilder, private api: ApiService, private route: Router, private toast: NgToastService) {
  ...
  addSale() {
    if (this.formGroup.value.qty > Number(this.productDetail[5])) {
      this.toast.error({ detail: "Out of Stock Message", summary: "Product is out of bound!!" })
    } else {
  ...
  valueSave() {
    ...
    this.api.postTypeRequest(`sale/save/`, sales).subscribe((res) => {
      this.toast.success({ detail: "Bill Generate Message", summary: "Bill has been created Successfully!!" })
      console.log(res);
      this.saleData = res;
      localStorage.setItem("id", this.saleData.id);

      this.route.navigateByUrl("admin/sale-report");
    })
  }
